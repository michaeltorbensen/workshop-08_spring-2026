...
drought_exp_clean <- drought_exp |> 
  # lowercase and removing spaces/adding only underscores
  clean_names() |> 
  # creating species name
  ...
  # giving full name to water type 
  mutate(water_treatment = recode_values(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |> 
  # changing the order of each species, organizing the names and data within columns 
  mutate(species_name = as_factor(species_name),
         # factors that show first are always going to be the reference level
...
slice_sample(
  drought_exp_clean, # clean data frame to display 10 rows 
  n = 10
)
...
labs(x = "Specific leaf Area (cm\U00B2 / g)",
     y = "total leaf mass in g",
     title = "As SLA Increases, Total g Increases")
...
labs(x = "water treatment type", 
     y = "total leaf mass in g", 
     title = "Plants well watered have higher mass than drought-stressed")
...
ggplot(data = drought_exp_clean, 
       mapping = aes(x = species_name, 
                     y = total_g, 
                     color = species_name)) + # coloring by species 
  # represent individuals in each species 
geom_jitter(height = 0) +
  # relabeling x- and y-axis and title 
  labs(x = "Species Name", 
       y = "Total leaf mass in g", 
       title = "Species differ in plant mass") + 
  # wrapping x-axis text
  scale_x_discrete(labels = label_wrap(10)) +
  # usuing customn colors 
  scale_color_manual(values = c(
    "Lotus scoparius" = lotsco_col, 
    "Pentstemon centranthifolius" = pencen_col, 
...
- residuals are normally distributed from QQ plot 
- residuals are homoscedastic from residuals vs fitted and scale-location plots 
...
- `water_treatmentWell watered`: model prediction for the difference in Lotus scoparius mass between well watered and drought stressed treatment. 
- `species_namepenstemon centranthifolius`: difference between penstemon mass in the well watered treatment and lotus scoparius mass in well watered treatment
- `species_nameEschscholzia californica`: difference between Eschscholzia californica mass in well watered treatment and lotus scoparius mass in well watered treatment
...
ggplot(data = drought_exp_clean, 
       mapping = aes(x = water_treatment, 
                     y = total_g, 
                     color = water_treatment)) + 
  # jitter: individual plats for each treatment in each species 
  geom_jitter(height = 0, 
              width = 0.1, 
              alpha = 0.4, 
              shape = 21) +
  # point range: model predictions with 95% CI
  ...
  facet_wrap(~ species_name, 
             nrow = 1) +  # all plots in a single row 
  # adds custom colors 
  scale_color_manual(values = c(
    "Drought stressed" = ds_col, 
    "Well watered" = ww_col
